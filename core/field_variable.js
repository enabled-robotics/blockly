...
Blockly.FieldVariable.dropdownCreate = function() {
  ...
  options.push(["New variable", "NEW_VAR"]);
...
Blockly.FieldVariable.prototype.onItemSelected = function(menu, menuItem) {
  ...
  if (this.sourceBlock_ && this.sourceBlock_.workspace) {
    ...
    if (id == Blockly.RENAME_VARIABLE_ID) {
      ...
      var filedName = this.name;
      var oldList = this.getOptions();

      Blockly.Variables.createVariableButtonHandler(workspace, null , this.defaultType_)
     var newList = this.getOptions();
     for (var i = 0; i < newList.length; i++) {
       if(newList[i][1] != oldList[i][1])
       {
         this.setValue(newList[i][1]);
         break;
       }
     }
